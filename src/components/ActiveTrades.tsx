          ...
          {settings?.activeStrategy && (
            <span className="inline-flex items-center gap-1.5 text-[10.5px] font-bold px-2.5 py-1 bg-gray-900 border border-gray-700 text-gray-200 rounded-lg">
              ...
              <span className="text-[#00e696]">
                {settings.activeStrategy === 'DELTA_CLIMAX' ? '⚡ Delta Climax Reversal'
                  : settings.activeStrategy === 'VOLATILITY_COMPRESSION' ? '💥 VCB Breakout'
                  : settings.activeStrategy === 'EARLY_COIL_BREAKOUT' ? '🔥 Early Coil Breakout'
                  : settings.activeStrategy === 'AUTO_REGIME' ? '🤖 Auto-Regime Adaptive'
                  : settings.activeStrategy === 'TREND_PULLBACK' ? '🎯 Trend Pullback'
                  : settings.activeStrategy === 'MACRO_RANGE_BREAKOUT' ? '📦 Macro Range Breakout'
                  : settings.activeStrategy === 'SMC_LIQUIDITY_SWEEP' ? '💧 Liquidity Sweep Reversal'
                  : '📊 Composite 10-Gate'}
              </span>
            </span>
          )}
