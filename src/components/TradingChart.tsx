...
export default function TradingChart({ coin, activePosition }: TradingChartProps) {
  ...
  useEffect(() => {
    ...
    // 7. VCB Compression Zone (Upper & Lower Boundary Rectangle)
    if (indicatorToggle.vcb && coin.vcb && coin.vcb.windowHigh > 0 && coin.vcb.windowLow > 0) {
      candlestickSeries.createPriceLine({
        price: coin.vcb.windowHigh,
        color: '#f59e0b',
        lineWidth: 2,
        lineStyle: LineStyle.Solid,
        axisLabelVisible: true,
        title: 'Upper Boundary (H_comp)',
      });

      candlestickSeries.createPriceLine({
        price: coin.vcb.windowLow,
        color: '#f59e0b',
        lineWidth: 2,
        lineStyle: LineStyle.Solid,
        axisLabelVisible: true,
        title: 'Lower Boundary (L_comp)',
      });
    }

    // 8. Active positions & VCB Setup Levels (Entry, SL, TP1, TP2, TP3)
    const tradeSetup = activePosition ? {
      direction: activePosition.direction,
      entry: activePosition.entryPrice,
      sl: activePosition.sl,
      tp1: activePosition.tp1,
      tp2: activePosition.tp2,
      tp3: activePosition.tp3,
      trailingStop: activePosition.trailingStop,
      isLive: true,
    } : (coin.vcb?.breakout ? {
      direction: coin.vcb.breakout.direction,
      entry: coin.vcb.breakout.entryPrice,
      sl: coin.vcb.breakout.sl,
      tp1: coin.vcb.breakout.tp1,
      tp2: coin.vcb.breakout.tp2,
      tp3: coin.vcb.breakout.tp3,
      trailingStop: null,
      isLive: false,
    } : (coin.sl && coin.tp1 ? {
      direction: (coin.direction === 'SHORT' ? 'SHORT' : 'LONG') as 'LONG' | 'SHORT',
      entry: coin.price,
      sl: coin.sl,
      tp1: coin.tp1,
      tp2: coin.tp2 || 0,
    ...
    if (tradeSetup) {
      ...
      const entryTitle = isLong ? 'Entry_long' : 'Entry_short';
      const slTitle = isLong 
        ? 'SL_long (min(L_comp, L_break) - buffer)' 
        : 'SL_short (max(H_comp, H_break) + buffer)';
      const tp1Title = isLong ? 'TP1_long (+2R)' : 'TP1_short (-2R)';
      const tp2Title = isLong ? 'TP2_long (+3R / Structure)' : 'TP2_short (-3R / Structure)';
      ...
      if (tradeSetup.tp3 && tradeSetup.tp3 > 0) {
        candlestickSeries.createPriceLine({
          price: tradeSetup.tp3,
          color: '#06b6d4',
          lineWidth: 1.5,
          lineStyle: LineStyle.Dashed,
          axisLabelVisible: true,
          title: isLong ? 'TP3_long (Runner)' : 'TP3_short (Runner)',
        });
      }
      ...
      if (typeof tradeSetup.trailingStop === 'number' && !isNaN(tradeSetup.trailingStop)) {
        candlestickSeries.createPriceLine({
          price: tradeSetup.trailingStop,
          color: '#f59e0b',
          lineWidth: 1.5,
          lineStyle: LineStyle.Dotted,
          axisLabelVisible: true,
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden p-5 shadow-lg flex flex-col space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-gray-950/40 p-3 rounded-lg border border-gray-800/60">
        ...
        <div className="flex flex-wrap items-center gap-1.5 text-[10px] font-mono leading-none">
          ...
          <button
            onClick={() => setIndicatorToggle({ ...indicatorToggle, vcb: !indicatorToggle.vcb })}
            className={`px-2 py-1 border rounded transition-all cursor-pointer font-semibold ${
              indicatorToggle.vcb ? 'bg-amber-500/20 border-amber-500 text-amber-300' : 'bg-gray-900 border-gray-800 text-gray-500'
            }`}
          >
            VCB BOX (H/L)
          </button>
      ...
      <div className="relative border border-gray-800 rounded-lg overflow-hidden flex flex-col bg-gray-950">
        {/* Floating VCB Compression Zone Banner */}
        {indicatorToggle.vcb && coin.vcb && coin.vcb.windowHigh > 0 && (
          <div className="absolute top-2.5 left-3 z-10 pointer-events-none flex flex-wrap items-center gap-2 text-[10px] font-mono bg-gray-950/85 backdrop-blur-md border border-amber-500/40 px-3 py-1.5 rounded-md shadow-lg text-gray-300">
            <span className="flex items-center gap-1 font-bold text-amber-400">
              <Target className="w-3.5 h-3.5" /> VCB Box
            </span>
            <span className="text-gray-600">|</span>
            <span>H_comp: <strong className="text-amber-300">${formatPrice(coin.vcb.windowHigh)}</strong></span>
            <span>L_comp: <strong className="text-amber-300">${formatPrice(coin.vcb.windowLow)}</strong></span>
            <span className="text-gray-600">|</span>
            <span className={coin.vcb.priorTrend === 'UPTREND' ? 'text-emerald-400 font-semibold' : coin.vcb.priorTrend === 'DOWNTREND' ? 'text-rose-400 font-semibold' : 'text-gray-400 font-semibold'}>
              Prior: {coin.vcb.priorTrend || 'NEUTRAL'}
            </span>
            {coin.vcb.isCompressed && (
              <span className="bg-amber-500/20 text-amber-300 border border-amber-500/50 px-1.5 py-0.5 rounded text-[9px] font-bold">
                COMPRESSION ZONE
              </span>
            )}
            {coin.vcb.breakout && (
              <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${coin.vcb.breakout.direction === 'LONG' ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/50' : 'bg-rose-500/20 text-rose-300 border border-rose-500/50'}`}>
                {coin.vcb.breakout.direction} BREAKOUT
              </span>
            )}
          </div>
        )}
