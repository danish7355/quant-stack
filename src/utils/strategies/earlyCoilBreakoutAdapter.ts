// src/utils/strategies/earlyCoilBreakoutAdapter.ts
// ─────────────────────────────────────────────────────────────────────────────
// Adapter for Early Coil Breakout strategy conforming to core/StrategySignal.
// ─────────────────────────────────────────────────────────────────────────────
import { evaluateEarlyCoilBreakout } from './earlyCoilBreakout.js';
...
export function evaluateEarlyCoilBreakoutAdapter(
  candles: any[],
  htfCandles: any[] = [],
  ...
  const result = evaluateEarlyCoilBreakout(candles, settings as any);
  ...
  const {
    ...
    reason = 'Early Coil Breakout',
  } = result;

  if (!rawDir || sl === undefined) return null;

  const entry = currentPrice || candles[candles.length - 1].close;
  ...
  const signalId = `${symbol}:${timeframe}:earlyCoilBreakout:${direction}:${candleTime}`;
