...
export function findCRSetup(candles: any[], settings: AppSettings): CRSignal | null {
    const minHistory = Math.max(settings.crClimaxLookback, settings.crAtrAveragePeriod, 30) + 5;
    if (candles.length < minHistory) {
        return null;
    }
    
    const enriched = enrichCandles(candles, settings);
    const cLive = enriched[enriched.length - 1];
    const cClosed = enriched[enriched.length - 2];
    const cPrev = enriched[enriched.length - 3];

    // Recent Swing High / Low check (last 25 candles)
    const recentLows = enriched.slice(Math.max(0, enriched.length - 25), enriched.length - 2).map(c => c.low);
    const lowestRecentLow = recentLows.length > 0 ? Math.min(...recentLows) : cClosed.low;
    const recentHighs = enriched.slice(Math.max(0, enriched.length - 25), enriched.length - 2).map(c => c.high);
    const highestRecentHigh = recentHighs.length > 0 ? Math.max(...recentHighs) : cClosed.high;

    // ==========================================
    // PATTERN A: SINGLE-BAR CLIMAX EXHAUSTION PIN BAR (Fast Sniper)
    // ==========================================
    // cClosed is a massive volume spike with huge rejection wick at swing extreme
    const isSingleBarVol = cClosed.volume >= (cClosed.avgVolume || 1) * 1.8;
    const singleRange = cClosed.high - cClosed.low;
    const singleUpperWick = singleRange > 0 ? (cClosed.high - Math.max(cClosed.open, cClosed.close)) / singleRange : 0;
    const singleLowerWick = singleRange > 0 ? (Math.min(cClosed.open, cClosed.close) - cClosed.low) / singleRange : 0;

    // Short Pin Bar: Swept recent highs with massive upper wick & high volume
    if (
        isSingleBarVol &&
        cClosed.high >= highestRecentHigh * 0.998 &&
        singleUpperWick >= 0.42 &&
        cLive.close < cClosed.high
    ) {
        const stopLevel = cClosed.high + (cClosed.atr * 0.2);
        return buildSignal('SHORT', enriched, cClosed, cClosed, cLive, stopLevel, settings);
    }

    // Long Hammer: Swept recent lows with massive lower wick & high volume
    if (
        isSingleBarVol &&
        cClosed.low <= lowestRecentLow * 1.002 &&
        singleLowerWick >= 0.42 &&
        cLive.close > cClosed.low
    ) {
        const stopLevel = cClosed.low - (cClosed.atr * 0.2);
        return buildSignal('LONG', enriched, cClosed, cClosed, cLive, stopLevel, settings);
    }

    // ==========================================
    // PATTERN B: TWO-BAR CLIMAX + IMMEDIATE REVERSAL
    // ==========================================
    const c1 = cPrev;   // Climax bar
    const c2 = cClosed; // Reversal bar
    const c3 = cLive;   // Live entry bar

    const bypassRejectionWick = settings.disabledGates?.['CR_rejectionWick'] || settings.disabledGates?.['cr_rejectionWick'];

    // SHORT SETUP (Parabolic Peak Reversal)
    if (
        c1.close > c1.open &&
    ...
    // LONG SETUP (Capitulation Bottom Reversal)
    if (
        c1.close < c1.open &&
