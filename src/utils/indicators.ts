...
export { calculateBollingerBands } from './strategies/rangeMeanReversion';
import { calculateBollingerBands, inRangeMarket } from './strategies/rangeMeanReversion';
...
export function runScoringEngine(
  candles: any[],
  params: any
): { score: number; direction: any; status: any; reason: string; indicators: any; gates: any; wmPattern: any; regime: any; sl?: number; tp1?: number; tp2?: number; tp3?: number } {
  ...
  const defaultGates = { g1: false, g2: false, g3: false, g4: false, g5: false, g6: false, g7: false, g8: false, g9: false, g10: false, blockReasons: [] as string[] };
  const defaultRegime: RegimeResult = { label: 'RANGE', score: 30, direction: 'NEUTRAL', componentScores: {} };
  const defaultInds = {
  ...
  const rangeCheck = inRangeMarket(closes, params.rangeSmaPct || 0.05, params.maxSmaSlope || 0.02);
  ...
  const atrCurrent = atrAll[idx] || close * 0.01;
  const atrSma5 = atrAll.slice(Math.max(0, idx - 5), idx).reduce((a, b) => a + b, 0) / 5;
  const volCurrent = volumes[idx];
  const vol20Ma = volSma[idx - 1] || 1;
  const rsi = rsi_all[idx];

  const prevIdx = idx - 1;
  const prevBar = candles[prevIdx] || candles[idx];
  const prevClose = prevBar.close;
  const prevBbLow = bb.lower[prevIdx];
  const prevBbHigh = bb.upper[prevIdx];
  const prevRsi = rsi_all[prevIdx] !== undefined ? rsi_all[prevIdx] : rsi;

  const currentBbLow = bb.lower[idx];
  const currentBbMid = bb.middle[idx];
  const currentBbHigh = bb.upper[idx];
  ...
  const completeIndDetails = {
    ...
    sma200: rangeCheck.smaValue
  };

  // --- 10 RANGING 1:3 R:R MEAN-REVERSION GATES WITH BYPASS SUPPORT ---
  ...
  // G3: Range Regime Filter Gate (Price within ±5% of 200-SMA or flat SMA slope)
  const g3Bypassed = isBypassed('COMPOSITE_g3', 'g3');
  const g3Raw = rangeCheck.inRange;
  gates.g3 = g3Raw || g3Bypassed;
  if (!g3Raw && !g3Bypassed) gates.blockReasons.push(`G3: Range Filter Failed (${rangeCheck.reason})`);

  // Determine potential direction from Bollinger Band & RSI extremes
  const rsiOversold = params.rsiOversold || 30;
  const rsiOverbought = params.rsiOverbought || 70;
  let detectedDir: 'LONG' | 'SHORT' | 'NEUTRAL' = 'NEUTRAL';
  if (prevClose <= prevBbLow || close <= currentBbLow * 1.002) {
    detectedDir = 'LONG';
  } else if (prevClose >= prevBbHigh || close >= currentBbHigh * 0.998) {
    detectedDir = 'SHORT';
  }

  // G4: Bollinger Band Extreme Setup Gate (Previous candle closed below lower BB or above upper BB)
  const g4Bypassed = isBypassed('COMPOSITE_g4', 'g4');
  let g4Raw = false;
  if (detectedDir === 'LONG' && prevClose <= prevBbLow) g4Raw = true;
  else if (detectedDir === 'SHORT' && prevClose >= prevBbHigh) g4Raw = true;
  gates.g4 = g4Raw || g4Bypassed;
  if (!g4Raw && !g4Bypassed) gates.blockReasons.push(`G4: Previous candle close did not breach Bollinger Bands (20, 2.0)`);

  // G5: RSI Extreme Filter Gate (RSI < 30 for LONG, RSI > 70 for SHORT)
  const g5Bypassed = isBypassed('COMPOSITE_g5', 'g5');
  let g5Raw = false;
  if (detectedDir === 'LONG' && prevRsi <= rsiOversold) g5Raw = true;
  else if (detectedDir === 'SHORT' && prevRsi >= rsiOverbought) g5Raw = true;
  gates.g5 = g5Raw || g5Bypassed;
  if (!g5Raw && !g5Bypassed) gates.blockReasons.push(`G5: RSI ${prevRsi.toFixed(1)} not at extreme (Long <= ${rsiOversold}, Short >= ${rsiOverbought})`);

  // G6: Confirmation Candle Gate (Current candle closes back inside Bollinger Bands)
  const g6Bypassed = isBypassed('COMPOSITE_g6', 'g6');
  let g6Raw = false;
  if (detectedDir === 'LONG' && close > currentBbLow) g6Raw = true;
  else if (detectedDir === 'SHORT' && close < currentBbHigh) g6Raw = true;
  gates.g6 = g6Raw || g6Bypassed;
  if (!g6Raw && !g6Bypassed) gates.blockReasons.push(`G6: Confirmation candle has not closed back inside Bollinger Bands`);

  // G7: Structure-Based Tight Stop Gate
  const g7Bypassed = isBypassed('COMPOSITE_g7', 'g7');
  const bandEntry = detectedDir === 'LONG' ? currentBbLow : currentBbHigh;
  const baseBandDist = Math.abs(close - bandEntry);
  const stopMult = params.stopMult || 1.5;
  const minStopDistance = Math.max(atrCurrent * 0.35, close * 0.003);
  let stopDistance = Math.max(stopMult * baseBandDist, minStopDistance);
  let g7Raw = stopDistance > 0 && stopDistance <= atrCurrent * 2.5;
  gates.g7 = g7Raw || g7Bypassed;

  // G8: Anti-Runaway / Volume Normalcy Gate
  const g8Bypassed = isBypassed('COMPOSITE_g8', 'g8');
  const barRange = Math.abs(candles[idx].high - candles[idx].low);
  const isRunaway = barRange > close * 0.03 || barRange > atrCurrent * 3.2;
  const isVolSpike = volCurrent > vol20Ma * 3.8;
  const g8Raw = !isRunaway && !isVolSpike;
  gates.g8 = g8Raw || g8Bypassed;
  if (!g8Raw && !g8Bypassed) gates.blockReasons.push(`G8: Runaway momentum / abnormal volume spike detected`);

  // G9: Funding Rate Guard
  const g9Bypassed = isBypassed('COMPOSITE_g9', 'g9');
  gates.g9 = true || g9Bypassed;

  // G10: 1:3 Fixed Risk/Reward Ratio Gate
  ...
  const mandatoryPass = gates.g1 && gates.g2 && gates.g3 && gates.g4 && gates.g5 && gates.g6 && gates.g8;
  const entryAllowed = mandatoryPass && detectedDir !== 'NEUTRAL';
  ...
  if (detectedDir === 'LONG') {
    finalSl = close - stopDistance;
    finalTp1 = Math.max(currentBbMid, close + (stopDistance * 1.5));
    finalTp2 = close + (3.0 * stopDistance);
    finalTp3 = close + (4.5 * stopDistance);
  } else if (detectedDir === 'SHORT') {
    finalSl = close + stopDistance;
    finalTp1 = Math.min(currentBbMid, close - (stopDistance * 1.5));
    finalTp2 = close - (3.0 * stopDistance);
    finalTp3 = close - (4.5 * stopDistance);
  }
  ...
  if (entryAllowed) {
    ...
    // Proximity score: closer to lower/upper band and oversold/overbought gives higher preparatory score
    const proximity = Math.min(prevRsi < 40 ? (40 - prevRsi) * 2 : 0, prevRsi > 60 ? (prevRsi - 60) * 2 : 0);
    finalScore = Math.max(20, Math.min(70, Math.round(30 + (passedGatesCount * 3) + proximity)));
  }

  return {
    score: finalScore,
    direction: entryAllowed ? detectedDir : 'NEUTRAL',
    status: entryAllowed ? 'ARMED' : (rangeCheck.inRange ? 'RANGE' : 'TRENDING'),
    reason: entryAllowed ? `1:3 R:R Mean-Reversion confirmed (${detectedDir})` : gates.blockReasons.join(' | ') || 'Scanning for Bollinger extreme re-entry',
